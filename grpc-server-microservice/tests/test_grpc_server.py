...
def test_fetch_birthday_user(grpc_stub):
    ...
    birthday_discount = Discount.select().where(
        Discount.title == "IS_BIRTHDAY_USER").get()
    try:
        # Getting birthday user
        birthday_user = User.raw(
            "SELECT * FROM public.user WHERE TO_CHAR(date_of_birth, 'MM-DD') LIKE %s", today).get()
        # Getting at least one product to send product_id
        product = Product.select().where(Product.description ==
                                         "MOCKED_PRODUCT_DESCRIPTION").get()
        ...
        if product and birthday_user:
            request = GetDiscountRequest(
                product_id=product.id, user_id=birthday_user.id)
...
def test_fetch_black_friday(grpc_stub):
    original_black_friday_discount = Discount.select().where(
        Discount.title == "IS_BLACK_FRIDAY").get()
    original_metadata = json.loads(original_black_friday_discount.metadata)

    # Setting blackfriday for today
    day = datetime.today().strftime('%d')
    month = datetime.today().strftime('%m')
    metadata = json.loads(Discount.select().where(
        Discount.title == "IS_BLACK_FRIDAY").get().metadata)
    metadata["day"] = day
