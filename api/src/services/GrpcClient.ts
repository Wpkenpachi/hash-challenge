...
export async function grpcGetDiscount(product_id: number, user_id: number) {
    const client = new IndividualProductDiscountClient(`${process.env.GRPC_SERVER_URI}:${process.env.GRPC_SERVER_PORT}`, grpc.credentials.createInsecure())
